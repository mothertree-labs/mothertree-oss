...
resource "cloudflare_dns_record" "lb1_a" {
...
resource "cloudflare_dns_record" "turn_a" {
...
resource "cloudflare_dns_record" "base_domain_cname" {
...
resource "cloudflare_dns_record" "matrix_fed_srv" {
  count   = local.use_cloudflare ? 1 : 0
  zone_id = var.cloudflare_zone_id
...
# MX mail host is mail.* (see cloudflare_dns_record.mail_a above) - no separate mail-relay record

# State migration: cloudflare_record → cloudflare_dns_record (Cloudflare provider v5)
moved {
  from = cloudflare_record.lb1_a
  to   = cloudflare_dns_record.lb1_a
}
moved {
  from = cloudflare_record.turn_a
  to   = cloudflare_dns_record.turn_a
}
moved {
  from = cloudflare_record.base_domain_cname
  to   = cloudflare_dns_record.base_domain_cname
}
moved {
  from = cloudflare_record.mail_mx
  to   = cloudflare_dns_record.mail_mx
}
moved {
  from = cloudflare_record.mail_a
  to   = cloudflare_dns_record.mail_a
}
moved {
  from = cloudflare_record.matrix_srv
  to   = cloudflare_dns_record.matrix_srv
}
moved {
  from = cloudflare_record.matrix_fed_srv
  to   = cloudflare_dns_record.matrix_fed_srv
}
moved {
  from = cloudflare_record.vpn_server
  to   = cloudflare_dns_record.vpn_server
}
