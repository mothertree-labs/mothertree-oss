...
if [ -n "$EXISTING_ID" ] && [ "$EXISTING_ID" != "null" ]; then
    ...
    if [ -z "$WARM_LB_IP" ]; then
        ...
        # so every later pipeline skipped deploy_infra and the first tenant
        # deploy then hard-aborted on the missing Secret, wedging the cluster
        # for every PR until the reaper destroyed it (pipelines #1679/#1680 on
        # cluster 623930, after the cancelled #1673-#1678).
        #
        # So also probe the LATE artifact — the Secret every tenant DB step
        # (db-init Jobs, the Nextcloud schema probes) reads. If it's absent,
        # the cluster is half-provisioned: mark it degraded so the repair block
        # below re-runs deploy_infra (idempotent; phase1-dev no-ops on the
        # existing cluster) to finish infra-db. A small retry absorbs a
        # transient kube-API blip so we don't trigger a needless ~10-min
        # redeploy; a genuinely-missing Secret falls through to repair.
        # Existence check only — probe with `-o name` and never bind the
        # password into a variable.
        WARM_DB_READY=""
...
# Orphaned persistent nextcloud_<tenant> DBs (the DB survives a cluster rebuild,
# the in-cluster nextcloud-identity Secret does not) are handled PER TENANT by
# apps/deploy-nextcloud.sh Step 4e (scripts/lib/nextcloud-db.sh, issue #548):
# on a cold start it probes the leased tenant's DB for a Nextcloud schema with
# an explicit verdict and drops a verified orphan before the install Job. The
# cluster-wide sweep that used to live here was removed on 2026-09-08: it
# listed the catalog through `kubectl run -i` and read a LOST attach result
# (exit 0, empty stdout) as "no databases" — a silent false negative in
# pipelines 1906 and 2108 — and, having no notion of the pool lease, it could
# drop a tenant another pipeline was installing at that moment.
