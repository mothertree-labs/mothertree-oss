<#--
    ...
    This page is shown when users authenticate with a passwordless passkey.
    Keycloak uses this template (not webauthn-authenticate.ftl) for passwordless WebAuthn flows.

    All styling is done with inline styles because Keycloak's base template for WebAuthn
    pages does not reliably render <style> tags from either the "header" or "form" sections.
-->
