...
function rcube_text_editor(config, id)
{
  ...
  if (config.mode == 'identity' || config.mode == 'response') {
    ...
    $.extend(conf, {
      plugins: 'autolink charmap code image link',
  ...
  // full-featured editor
    ...
    $.extend(conf, {
      plugins: 'autolink charmap code directionality link lists image media'
        + ' table searchreplace'
    });
  }
    ...
    if (rcmail.env.task == 'mail' && this.editor) {
      ...
      if (textarea.tabIndex > 0) {
        ...
        if (x !== null) {
          ...
          // tabfocus plugin removed in TinyMCE 6+
        }
      }
  ...
  this.toggle = function(ishtml, noconvert)
  {
    ...
    if (ishtml) {
      ...
      var init_editor = function(data) {
        ...
        input.val(data);
        var addConf = Object.assign({}, conf, {selector: '#' + ref.id});
        tinymce.init(addConf);
      };
  ...
  {
    if (this.editor) {
      // spellchecker plugin removed in TinyMCE 6+; browser_spellcheck handles it
