...
Deploy scripts use `mt_apply` (wraps `kubectl apply`, runs a server-side `kubectl diff` of the same manifest first and flags a change only when the live object actually differs or is absent) and `mt_restart_if_changed` (only does `kubectl rollout restart` when a change was flagged) to avoid restarting pods unnecessarily during deploys. This prevents disrupting active Jitsi calls, document editing sessions, and user login sessions.

**Why diff, not the apply output**: client-side `kubectl apply` prints "configured" whenever it *sends* a patch, even one the API server turns into a no-op — every Secret rendered by `kubectl create --dry-run=client -o yaml` (`creationTimestamp: null`), every `stringData` Secret, and any manifest whose quantities the server normalises (`cpu: 2000m` → `2`). Grepping that word made Ollama (1.3 GB model re-pull from S3), Jitsi Prosody, Roundcube, both portals and Open WebUI restart on every deploy. `kubectl diff` output is discarded (it would print Secret data into logs); if diff itself cannot run (rc > 1) `mt_apply` warns and falls back to the output grep, which only ever over-restarts. Unit tests: `scripts/lib/tests/mt-apply.test.sh`.
...
**Debugging deploy issues**: If a service isn't picking up new config after a deploy, the conditional restart system may be the cause. A ConfigMap/Secret applied with plain `kubectl apply` (not `mt_apply`) is invisible to the tracker; a `mt_apply` line followed by "No config changes detected" means the server-side diff found the live object identical. To force a restart, run `kubectl rollout restart <resource> -n <namespace>` manually. A "kubectl diff unavailable" warning in a deploy log means detection fell back to the output grep for that resource. Detection is stateless: if a deploy applied a changed Secret and died before its `mt_restart_if_changed`, the next deploy sees no difference and will not restart the consumer — restart it by hand.

**When modifying deploy scripts**: If you add a new ConfigMap or Secret that a pod depends on, make sure its `kubectl apply` is wrapped with `mt_apply` so changes are tracked. If you add it with plain `kubectl apply`, config changes won't trigger a pod restart. `mt_apply` needs a `-f` source (file, `-` for stdin, or `<(...)`); it reads the manifest once and feeds it to both diff and apply. Never pipe into or out of it (`gen | mt_apply kubectl apply -f -` and `mt_apply … | tee` both run it in a subshell and lose the flag; use `-f <(gen)`, a heredoc, or a redirect instead — `scripts/lib/tests/mt-apply-callsites.test.sh` fails CI on either shape) and only pass flags both `kubectl diff` and `kubectl apply` accept. An empty manifest is rejected (rc 2), and `mt_restart_if_changed` does not restart a target whose rollout is already in flight (the pods it is creating start after the config apply, so they already have it).
...
Policy: **every minor/patch/digest bump lands in ONE weekly PR** (Wednesday) on branch `renovate/weekly-batch` — each PR costs a full dev deploy plus 10 e2e shards, so PR count is the cost driver. **Major bumps only appear on the Dependency Dashboard issue and need a checkbox**, then open as their own PR. The named groups (`keycloak`, `jitsi`, `docs-impress`, `nextcloud`, `llm`, `tailscale-mesh`) still bind their majors in lock-step. The batching rules must stay **last** in `packageRules` — the last matching rule wins, which is what lets the batch override those groups for non-majors.

If one bump in the batch is bad, do **not** push a revert to the PR: Renovate stops updating a branch someone else committed to. Add a temporary `allowedVersions` rule for that package (as the Stalwart freeze does) and Renovate rebuilds the batch without it. **Every such hold needs a re-eval date and a tracking issue in its comment**: `allowedVersions` also hides the package from the Dependency Dashboard, so an undated hold silently suppresses that package's security updates forever.
...
- Keycloak image: `quay.io/keycloak/keycloak:26.7.4`
