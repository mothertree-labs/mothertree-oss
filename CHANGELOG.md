...
- `mt_apply` (the conditional-restart change tracker in `scripts/lib/common.sh`)
  ...
  Client-side apply prints "configured" whenever it *sends* a patch, even one
  the API server turns into a no-op — every Secret generated by
  `kubectl create --dry-run=client -o yaml`, every `stringData` Secret, and
  any manifest with a quantity the server normalises (`cpu: 2000m` → `2`).
  That made `mt_restart_if_changed` restart Ollama (with a 1.3 GB model
  re-download from the S3 model cache, a 38 Mbit/s inbound burst on the node
  and 5–12 min of deploy time), Jitsi Prosody, Roundcube, both portals and
  Open WebUI on **every** deploy of every environment — exactly the
  disruption the tracker exists to prevent. Diff output is discarded so
  Secret data never reaches deploy logs; when diff cannot run (rc > 1) the
  old output grep is used with a warning. Covered by
  `scripts/lib/tests/mt-apply.test.sh`.
