...
/**
 * Poll a specific folder for an email matching the subject and return its raw MIME source.
 * Returns null if IMAP is not configured or the email is not found within the timeout.
 */
export async function fetchFromFolder(opts: {
  ...
  while (Date.now() - start < timeoutMs) {
    let matchUid: number | null = null;

    // Phase 1: Find matching UID by envelope
    {
      const client = await connectAsMaster(opts.userEmail, config);
      try {
        ...
        const uids = await client.search({ all: true });
        if (uids && uids.length > 0) {
          for await (const msg of client.fetch(uids.slice(-10), { uid: true, envelope: true })) {
            const subject = (msg.envelope?.subject || '').toLowerCase();
            if (subject.includes(needle)) {
              matchUid = msg.uid;
              break;
        ...
        console.log(`  [imap] fetchFromFolder poll error: ${(err as Error).message}`);
      } finally {
        try { client.close(); } catch {}
      }
    }

    // Phase 2: Download the matching message (separate connection)
    if (matchUid !== null) {
      for (let attempt = 1; attempt <= 3; attempt++) {
        const client = await connectAsMaster(opts.userEmail, config);
        try {
          await client.mailboxOpen(opts.folder);
          const dl = await client.download(String(matchUid), undefined, { uid: true });
          const chunks: Buffer[] = [];
          for await (const chunk of dl.content) {
            chunks.push(chunk as Buffer);
          }
          ...
          console.log(`  [imap] fetchFromFolder download attempt ${attempt}: ${(err as Error).message}`);
        } finally {
          try { client.close(); } catch {}
        }
      }
