...
for extra in scripts/create_env scripts/deploy_infra scripts/check-tailscale-keys scripts/infra-health-gate scripts/verify-alerting scripts/verify-reflector scripts/check-health apps/manifests/tailscale-key-rotator/rotate.sh; do
  if [ -f "$extra" ]; then
