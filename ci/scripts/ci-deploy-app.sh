...
case "$MODE" in
  ...
  remint-caldav-tokens)
    ...
    # Helm chart), NOT in tenant config. We try to read them the same way
    # deploy-calendar-automation.sh does, but on the re-mint path the password
    # is NOT functionally required: it's only used for the optional admin
    # password drift-correction (occ user:resetpassword, `|| true`), which
    # deploy-dev-calendar already performed ~minutes earlier in this same
    # pipeline. The real token mint authenticates as root (occ user:add) /
    # www-data (create-caldav-tokens.php), never as admin. So a read failure
    # here is a warning, not fatal — failing hard would skip the re-mint and
    # leave e2e-mailrt without a token (the exact regression this fix targets).
    ...
    if [ -z "$NEXTCLOUD_ADMIN_PASSWORD" ]; then
      echo "WARNING: could not read Nextcloud admin password from secret 'nextcloud' in $NS_FILES"
